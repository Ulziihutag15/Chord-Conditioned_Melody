...
class Preprocessor:
    ...
    def preprocess(
        ...
        for split in data_split:
            ...
            shutil.rmtree(split_sub_dir.encode_tmp, ignore_errors=True)
            Path(split_sub_dir.encode_tmp).mkdir(exist_ok=True, parents=True)

            source_dirs = [split_sub_dir.raw]
            if use_augmentation:
                self.augment_data(
                ...
                source_dirs.append(split_sub_dir.augmented)
            ...
            np.save(
                str(default_sub_dir.encode_npy.joinpath(f"input_{split}.npy")),
                self.to_object_array(input_npy),
                allow_pickle=True,
            )
            np.save(
                str(default_sub_dir.encode_npy.joinpath(f"target_{split}.npy")),
                self.to_object_array(target_npy),
                allow_pickle=True,
            )
    ...
    def _preprocess_midi_chunk(
        ...
        copied_sample_infos_chunk.extend(
            [
                ...
                if sample_id not in parent_sample_ids and sample_id.split("_")[0] in parent_sample_ids
            ]
        )
        ...
        for sample_info_idx, sample_info in enumerate(copied_sample_infos_chunk):
            ...
            if sample_info.get("augmented", False):
                ...
                copied_sample_info = self.prepare_sample_info(copied_sample_info)
                midi_path = sample_id_to_path.get(copied_sample_info["id"])
                if midi_path is None:
                    continue
                try:
                    ...
                    print(f"num measures not allowed: {midi_path}")
                    continue
                output_dir = encode_tmp_dir.joinpath(f"{idx:04d}")
                output_dir.mkdir(exist_ok=True, parents=True)
                try:
    ...
    @staticmethod
    def prepare_sample_info(sample_info: Dict[str, Any]) -> Dict[str, Any]:
        copied_sample_info = copy.deepcopy(sample_info)
        copied_sample_info["rhythm"] = copied_sample_info.get(
            "rhythm", copied_sample_info.get("sample_rhythm")
        )
        copied_sample_info["is_incomplete_measure"] = copied_sample_info["num_measures"] % 4 != 0
        return copied_sample_info
    ...
    @staticmethod
    def to_object_array(arrays: List[np.ndarray]) -> np.ndarray:
