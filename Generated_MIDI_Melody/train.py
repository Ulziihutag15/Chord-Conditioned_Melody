...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="auto",
        help="device to train on; auto uses cuda, then mps, then cpu",
    )
    parser.add_argument(
        "--work_dir",
        type=str,
...
def train():
    ...
    for batch, (data, target, reset_mems, batch_token_num) in enumerate(
        ...
        get_base_model(model).temperature = 1.0
        ...
        grad_norm = torch.nn.utils.clip_grad_norm_(
            get_base_model(model).parameters(), cfg.TRAIN.clip
        )
        ...
        if train_step % cfg.TRAIN.log_interval == 0:
            reduce_tensor(log_train_loss)
            reduce_tensor(log_grad_norm)
            reduce_tensor(log_token_num)

            log_train_loss /= log_token_num
            log_grad_norm /= cfg.TRAIN.log_interval * world_size
        ...
        if train_step % cfg.TRAIN.eval_interval == 0:
            ...
            reduce_tensor(val_token_num_pt)
            reduce_tensor(val_total_nll_pt)
            ...
            if not best_val_nll or val_nll < best_val_nll:
                ...
                def calculate_test_nll_during_training(test_iter):
                    ...
                    reduce_tensor(test_token_num_pt)
                    reduce_tensor(test_total_nll_pt)
...
if args.device == "auto":
    ...
    device = torch.device(args.device, args.local_rank) if args.device == "cuda" else torch.device(args.device)
...
exp_time = torch.tensor(time.time(), dtype=torch.float32).to(device)
if is_distributed():
...
val_iter = dataset.eval_iterator(
    ...
    world_size=world_size if is_distributed() else 0,
)
...
# MLE optimizer
local_lr = cfg.TRAIN.lr / world_size
